...
### 개요
일기의 감정을 분류하는 **Logistic Regression** 기반 텍스트 분류 모델입니다. ML 모델의 분석 결과는 정확하지 않으므로로 **참고용**으로 제공되며, 실제 마을 시스템에는 GPT-4o-mini의 분석 결과가 사용됩니다.
...
- **사랑**: 키워드 기반 매핑 (ML 모델에 직접적으로는 없지만, "기쁨"에서 키워드 감지 시 30% 재분배)
- **놀람**: "당황"에서 **50%** 매핑
...
- **평가 지표**: `backend/services/models/metrics.json`
  - 정확도 (Accuracy)
  - 각 감정별 Precision, Recall, F1-Score
- **Confusion Matrix**: `backend/services/models/confusion_matrix.png`
  - 테스트 데이터에 대한 혼동 행렬 시각화
- **검증 데이터 평가**: `backend/services/models/val_confusion_matrix.png`, `val_metrics.json`
