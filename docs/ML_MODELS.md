...
moodtown은 두 가지 ML 모델을 사용합니다.

## 1. 감정 분석 모델 (Emotion Classification)

### ❤︎ 개요
일기의 감정을 분류하는 **Logistic Regression** 기반 텍스트 분류 모델입니다.  
ML 모델의 분석 결과는 정확하지 않으므로 **참고용**으로 제공되며, 실제 마을 시스템에는 GPT-4o-mini의 분석 결과가 사용됩니다.
...
- **매핑 상세 설명**:
  ...
  - **"상처"** → "슬픔"에 0.5배로 추가 (슬픔 과다 증가 방지)
  - **"불안"** → "두려움"으로 0.7배 스케일링
  - **"사랑"** → "기쁨" 점수가 있을 때, 사랑 관련 키워드(사랑, 좋아, 애정, 그리움, 소중 등)가 감지되면 "기쁨" 점수의 30%를 "사랑"으로 재분배

- **점수 조정 규칙**:
  - 부정적 키워드가 **3개 이상**이고 긍정 키워드보다 많을 때만 "기쁨" 점수 감소 (과도한 감소 방지)
  - 긍정적 키워드가 부정 키워드의 70% 이상이면 "기쁨" 점수 증가 허용
  - "슬픔" 점수가 0.4 이상이면 초과분의 30%를 "기쁨"으로 재분배 (슬픔 과다 증가 방지)
  - "두려움" 점수 최대값 0.25로 제한 (과도한 예측 방지), 초과분의 10%를 "기쁨"으로 재분배
...
- **Confusion Matrix**: `backend/services/models/confusion_matrix.png`
  ...
  - ![혼동행렬](../backend/services/models/confusion_matrix.png)
...
### ❤︎ 개요
일기 간 유사도를 계산하여 비슷한 감정이나 내용의 일기를 찾아주는 **Doc2Vec** 기반 문서 임베딩 모델입니다.  
