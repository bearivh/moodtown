...
#### 유사한 일기 찾기
- **Doc2Vec 모델 기반**으로 선택한 일기와 유사한 감정/내용의 과거 일기 검색
...
- **`treeStateCache`**: 행복 나무 상태 캐시
- **`wellStateCache`**: 스트레스 우물 상태 캐시
...
### transformers 4.39.3+
- **용도**: KLUE-RoBERTa 기반 감정 분류 모델
- **알고리즘**: Transformers (RobertaForSequenceClassification)

### Gensim 4.3.0+
- **용도**: Doc2Vec 기반 유사 일기 검색 모델
- **알고리즘**: Doc2Vec (PV-DM)
...
│   │   │   ├── diary_similarity_metadata.json
│   │   │   ├── emotion_ml.joblib
│   │   │   ├── label_counts.train.json
│   │   │   ├── metrics.json
│   │   │   └── test_predictions.jsonl
│   │   ├── train_diary_similarity.py  # Doc2Vec 모델 학습 스크립트
│   │   └── train_emotion_ml.py        # 감정 분석 모델 학습 스크립트
│   ├── core/                          # 공통 모듈
│   │   ├── __init__.py
│   │   └── common.py                  # 공통 유틸리티
...
GPT-4o-mini와 ML 모델을 활용한 정확한 감정 분석 시스템입니다.
...
### 하이브리드 극성 판단
- **Rule-based 분석**: 키워드 패턴 매칭
  - 긍정 키워드: 좋은 소식, 반가운, 설레는 등
  - 부정 키워드: 충격, 실망, 창피함, 수치심 등
- **GPT 기반 분석**: 문맥 이해
  - 전체 문맥을 고려한 극성 판단
  - 더 정확한 분류
...
Doc2Vec 기반으로 과거 유사한 감정의 일기를 찾는 기능입니다.

### 유사도 계산
- **텍스트 유사도**: Doc2Vec 모델로 문서 벡터화 후 코사인 유사도 계산
- **감정 점수 유사도**: 두 일기의 감정 점수 분포 비교
- **최종 유사도**: (텍스트 유사도 × 0.6) + (감정 점수 유사도 × 0.4)
...
| 리소스 | 설명 |
|--------|------|
| transformers 4.39.3+ | KLUE-RoBERTa 기반 감정 분류 모델 |
