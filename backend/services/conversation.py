    ...
    try:
        ...
        for emotion in EMOTION_KEYS:
            if emotion in CHARACTERS:
                participating_characters.append({
                    ...
                    "emotion": emotion,
                    "role": "main" if emotion in main_emotions else "reactive"
                })
        
        # 참여할 주민 목록 문자열 생성 (주요/반응 구분)
        main_characters_list = ", ".join([f"{char['name']}({char['emotion']})" for char in participating_characters if char['role'] == 'main'])
        reactive_characters_list = ", ".join([f"{char['name']}({char['emotion']})" for char in participating_characters if char['role'] == 'reactive'])
        all_characters_list = ", ".join([f"{char['name']}({char['emotion']})" for char in participating_characters])
        ...
        for emotion in EMOTION_KEYS:
            if emotion in CHARACTERS:
                ...
                role_text = "⭐ 자신의 감정을 주로 표현" if emotion in main_emotions else "💬 다른 감정들에게 반응만 (위로/동조/반박)"
                character_descriptions.append(
                    f"- {name}({emotion}) [{role_text}]: {style}\n"
                    f"  말투 특징: {speech_hints}"
                )
        
        character_roles_text = "\n".join(character_descriptions)
        ...
        prompt = (
            ...
            "7) 감정 주민들은 개별 인격이 아니라 '사용자의 감정 자체'입니다.\n"
            "8) 각 주민은 \"나도 예전에 그런 적 있어\", \"전에 겪어봤지\", \"옛날에\" 등의 표현을 해서는 안 됩니다.\n\n"
            f"🎭 참여할 주민 목록 (반드시 모두 참여): {all_characters_list}\n\n"
            f"⭐ 주요 감정 (자신의 감정을 주로 표현): {main_characters_list}\n\n"
            f"💬 반응 감정 (반응만, 위로/동조/반박): {reactive_characters_list if reactive_characters_list else '(없음)'}\n\n"
            "🧩 각 주민의 감정별 역할과 말투 (반드시 준수)\n\n"
            f"{character_roles_text}\n\n"
            "📌 역할 규칙 (중요!)\n\n"
            ...
            "주요 감정: 빨강이(분노 50%), 파랑이(슬픔 30%)\n"
            "반응 감정: 노랑이(기쁨 0%), 초록이(사랑 0%)\n"
            "대화:\n"
            "- 빨강이(분노): \"으휴! 그 상황에서 너무 짜증났어! 왜 이렇게 무례한 거야?\" (자신의 감정 표현)\n"
            "- 파랑이(슬픔): \"그래도 나를 너무 막 대하는 것 같아... 마음이 무겁네.\" (자신의 감정 표현)\n"
            "- 노랑이(기쁨): \"그래도 너무 화내지는 말자! 기분만 더 안 좋아지잖아.\" (반응만, 자신의 감정 표현 안 함)\n"
            "- 초록이(사랑): \"우리 친구도 나쁜 의도는 아니었을 거야.\" (반응만, 위로)\n"
            "⚠️ 주의: 주요 감정은 자신의 감정을 표현하고, 반응 감정은 반응만 합니다!\n\n"
            "📘 일기:\n\n"
            f"{diary_text}\n\n"
